...
## Current Deployment And Source Release Scope (2026-09-08)
...
The v1.0.0 source-release scope is separate from the deployed host. A source
release MUST identify its exact candidate commit and supported Compose build
path independently of the host version recorded in the live handoff. This
scope does not upgrade the current host, and the full signed-package, Windows,
and cross-host operational workflows are not supported or advertised release
paths for v1.0.0.

## Purpose

Internal deployment readiness defines the controlled-LAN runtime profile,
role-scoped configuration, dependency health, the scoped source-release
contract, and the separate full-release evidence path for formal internal
exams. The current minimal Mac runtime is a scoped deployment decision; it
does not satisfy the full signed/backup-enabled release gate below.
