> **Current scope (2026-09-08):** The live deployment is the scoped minimal Mac runtime at `192.168.2.225`. This change records implemented application hardening plus the future Windows Docker Desktop + WSL2 adapter and cutover contract. Its remaining Windows tasks are not current minimal Mac blockers; current Mac acceptance does not complete them.

## Why

> **Historical target update (2026-08-07):** The archived `2026-08-27-abandon-formal-host-cutover` record selected macOS as the full formal target/source. This change preserves the Windows Docker Desktop + WSL2 adapter and its real-host acceptance track for a future Mac-to-Windows migration; current minimal Mac evidence does not complete the remaining Windows tasks or make either full host path ready without its own acceptance gates.
...
The active set currently contains this change, `harden-internal-deployment-readiness`, and `remediate-repository-security-findings`. The archived Mac host-portability and open-email-registration records preserve their historical capability ownership. This change owns the implemented application hardening and future Windows adapter, while tasks 12.4 and 12.5 remain pending native AMD64 Mac-to-Windows cutover acceptance. Never treat current minimal Mac evidence as Windows completion or archive one change by overwriting another change's specs.
