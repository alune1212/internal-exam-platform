...
- [x] 2.5 Add disposable macOS staging up/down, schemaVersion-2 acceptance, evidence capture, separate loopback ports, project-name protection, real external browser/SMTP/capacity producers, real second-copy restore-smoke, and safe cleanup without touching formal volumes.
...
- [x] 3.5 Add macOS formal preflight covering release and image identity, architecture, Docker/Compose, explicitly disabled Resource Saver, configuration permissions, fixed bind/CORS, service/worker health, migration, disk, SMTP, paired backup, independent encrypted second-copy evidence, browser smoke evidence, target-maintenance mode, and explicit privileged PF/network-time evidence.
...
- [ ] 8.2 Build and start the disposable native ARM64 macOS staging project, migrate to head, and prove route separation, approved-CIDR/pf port negatives, exact six-service restart recovery, real SMTP, daily opportunity backup degraded/skipped handling, real second-copy restore-smoke, diagnostics, security, browser E2E report, exact-image capacity, and schemaVersion-2 checksummed evidence.
- [ ] 8.3 Install/enable and real-test the mandatory formal LaunchAgent, then commission the initial Mac writer through `Prepare --empty-dataset` → schemaVersion-2 staging durable bundle → private maintenance `Capture-FormalBrowserSmokeEvidence` → designated-account privileged evidence → `Activate` generation-1 fence/final paired backup+second copy/restore drill/target-maintenance preflight/pending barrier/terminal evidence/public start, run desktop and phone UAT, perform host-restart retry/lock checks, close sessions, and retain the final evidence bundle. Cross-host `prepare-cutover`/`accept-cutover` remains a future migration flow; all real external gates remain blocking.
