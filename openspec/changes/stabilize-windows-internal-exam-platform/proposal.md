...
There are four active change directories. `harden-internal-deployment-readiness` is a completed baseline but is intentionally not auto-archived in this round; it must not reclaim host-specific capability ownership. `support-macos-formal-host-portability` owns the selected Mac formal source/target, shared portability identity, and cross-host `prepare-cutover`/`accept-cutover` contract. `open-email-registration-with-invited-exams` owns platform-account, roster, invitation, and account-migration behavior while preserving the selected-host writer and evidence boundary. This change owns the implemented application hardening and future Windows adapter, while tasks 12.4 and 12.5 remain pending native AMD64 Mac-to-Windows cutover acceptance. Archive separately and in evidence order: leave harden active for this round, archive support only after real Mac acceptance, archive the account change only after its independent implementation and live migration gates pass, and archive this Windows change only after real Windows staging, cutover, UAT, and evidence complete 12.4/12.5 (with 12.6). Never treat Mac evidence as Windows completion or archive one change by overwriting another change's specs.
