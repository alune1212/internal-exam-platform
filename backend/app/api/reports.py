...
def get_score_report(db: Session = Depends(get_db)) -> ApiResponse[list[ScoreReportRow]]:
...
def get_question_accuracy(db: Session = Depends(get_db)) -> ApiResponse[list[QuestionAccuracyRow]]:
...
def get_wrong_questions(db: Session = Depends(get_db)) -> ApiResponse[list[WrongQuestionRow]]:
...
def get_absent_candidates(db: Session = Depends(get_db)) -> ApiResponse[list[AbsentCandidateRow]]:
    return ApiResponse(data=report_service.get_absent_candidates(db))
