...
from app.schemas.question import QuestionCreate, QuestionImportResult, QuestionRead, QuestionUpdate
...
def create_question(payload: QuestionCreate, db: Session = Depends(get_db)) -> ApiResponse[QuestionRead]:
...
def delete_question(question_id: int, db: Session = Depends(get_db)) -> ApiResponse[dict[str, int]]:
