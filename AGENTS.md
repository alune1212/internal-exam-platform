...
- Preserve fixed-paper semantics: non-empty `exam.question_rule` with `question_count` generates/reuses a fixed 60-question paper via `fixed_question_ids`; empty `{}` keeps the legacy all-active behavior.
