...
第一阶段的路由、页面和 service 边界已经建立，题库 Excel 导入、应参人员 Excel 导入、考试配置、开始考试快照、答案暂存和提交判分已经具备入库闭环。到时自动提交调度、排名和报表 SQL 仍是后续工作。接手实现真实业务时优先查看 `docs/handoff.md`。
...
3. 实现到时自动提交调度。
4. 完成成绩排名、题目正确率、错题排行、未参加人员报表查询。
5. 增加必要的管理员会话保护，但不引入复杂 RBAC。
