...
第一阶段已经形成可运行的考试闭环：后端 API、数据库模型、迁移、前端页面、Docker Compose 和项目文档。考试默认按固定 60 题试卷出题并保留题目快照；更复杂的抽题配置向导、完整认证体系、Excel 导出美化和更精细的考试人员范围管理会在后续阶段补齐。
...
- 开始考试时按 `question_rule` 生成固定 60 题试卷，创建 attempt 和题目快照，答案暂存入库，提交后按快照自动判分
...
第一阶段的路由、页面和 service 边界已经建立，题库 Excel 导入、应参人员 Excel 导入、考试配置、固定 60 题试卷、开始考试快照、答案暂存、提交判分、自动提交、排名和基础报表已经具备入库/查询闭环。仍未完成的是导入失败报告下载、考试与应参人员范围的明确关联、报表导出文件和正式会话保护。接手实现真实业务时优先查看 `docs/handoff.md`。
