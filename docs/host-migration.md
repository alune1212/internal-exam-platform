...
迁移只允许使用一个逻辑数据集、一个正式 writer 和一个经过校验的 release。Mac 与 Windows 不得同时暴露候选写入口；开发 checkout、staging、旧宿主和 target host 不得共享正式 volume。
...
5. 停止整个 Mac formal Compose project（candidate/operator gateway、frontend、backend、worker、database），并记录停止时间、最后 commit、最后 backup ID、writer generation 和“source stopped”状态。停止后不得重新暴露候选入口。
6. 目标 Windows 未完成 restore、migration、服务、网络、SMTP、浏览器和容量证据前，不得对考生开放。
...
4. 仅在 staging evidence、preflight、paired backup 和人工批准均通过后，把候选入口切换到 Windows。记录 target writer generation、target release/commit、target host OS/architecture 和 source stopped 证据。
5. 切换后关闭旧 Mac 的 LaunchAgent/Compose recovery，并保持旧 Mac 断开候选入口；只允许 Windows writer。
...
若 Windows staging/target 尚未开放候选写入，可停止 target，回到已停止的 Mac source，重新验证 source release、最后 paired backup 和 source writer generation，再由人工批准恢复 Mac。不能把“target 容器启动失败”当作双写理由。
...
以下任一项缺失即停止迁移或开考：第二副本不可校验；source 仍可写；存在 `in_progress` attempt；target 使用错误架构或未验证 release；两宿主同时暴露候选入口；固定 IP/CORS/pf 未重新验证；真实 SMTP、UAT、capacity 或 restore drill 失败；LaunchAgent/Windows recovery 自动批准开考；诊断或证据含 secret/PII。
