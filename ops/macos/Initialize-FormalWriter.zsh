...
# A release copy cannot bootstrap its own trust root.  Refuse it before any
# bundle-controlled helper is sourced; Prepare/Activate are owner-operated
# actions from the exact clean checkout that produced the installed release.
self_release_path="${SCRIPT_DIR:h:h}"
if [[ -f "$self_release_path/release-manifest.json" ]]; then
  print -u2 -- "macOS operation failed: first-writer commissioning must run from the trusted checkout"
  exit 1
fi
...
macos_assert_clean_trusted_checkout() {
  local actual_root source_commit dirty
  actual_root="$(git -C "$REPO_ROOT" rev-parse --show-toplevel 2>/dev/null)" || macos_die "trusted checkout is not a Git worktree"
  [[ "${actual_root:A}" == "${REPO_ROOT:A}" ]] || macos_die "trusted checkout root is unexpected"
  source_commit="$(git -C "$REPO_ROOT" rev-parse --verify HEAD 2>/dev/null)" || macos_die "trusted checkout has no exact HEAD"
  [[ "$source_commit" =~ '^[0-9a-fA-F]{40}$' ]] || macos_die "trusted checkout HEAD is invalid"
  git -C "$REPO_ROOT" diff-index --quiet HEAD -- || macos_die "trusted checkout has tracked changes; use the clean release source"
  dirty="$(git -C "$REPO_ROOT" status --porcelain --untracked-files=all)"
  [[ -z "$dirty" ]] || macos_die "trusted checkout has untracked files; use the clean release source"
  typeset -g BOOTSTRAP_SOURCE_COMMIT="${source_commit:l}"
}
...
bootstrap_assert_fresh_state() {
  local state_path artifact
  if [[ ! -e "$intent_path" && ! -e "$intent_path.sha256" ]]; then
    for state_path in "$MACOS_CURRENT_STATE" "$MACOS_PREVIOUS_STATE" "$identity_path" "$lineage_path" "$volume_override" "$volume_override.sha256"; do
      [[ ! -e "$state_path" ]] || macos_die "fresh writer root already contains prior state: ${state_path:t}"
    done
  else
...
bootstrap_resolve_release() {
  ...
  [[ "${release_commit:l}" == "${BOOTSTRAP_SOURCE_COMMIT:-}" ]] || macos_die "release commit does not match the clean trusted checkout"
...
bootstrap_validate_current_shape() {
  ...
  [[ "$(macos_json_get "$MACOS_CURRENT_STATE" kind 2>/dev/null || true)" == formal-writer-current && "$(macos_json_get "$MACOS_CURRENT_STATE" applicationVersion 2>/dev/null || true)" == "$BOOTSTRAP_RELEASE_VERSION" && "$(macos_json_get "$MACOS_CURRENT_STATE" gitCommit 2>/dev/null || true)" == "$BOOTSTRAP_RELEASE_COMMIT" && "$current_dataset" == "$BOOTSTRAP_DATASET_ID" && "$current_host" == "$BOOTSTRAP_HOST_ID" && "$current_generation" == 1 && "$current_release" == "$BOOTSTRAP_RELEASE_PATH" ]] || macos_die "formal writer current state is not bound to the exact generation-1 reservation"
...
bootstrap_write_derived_state() {
  local digest="$(macos_sha256 "$intent_path")" release_json identity_json current_json derived_path
  ...
  for derived_path in "$identity_path" "$MACOS_CURRENT_STATE"; do
    if [[ -f "$derived_path" ]]; then
      plutil -convert json -o - -- "$derived_path" >/dev/null 2>&1 || macos_die "derived writer state is invalid"
      [[ "$(macos_json_get "$derived_path" datasetId 2>/dev/null || true)" == "$BOOTSTRAP_DATASET_ID" && "$(macos_json_get "$derived_path" hostId 2>/dev/null || true)" == "$BOOTSTRAP_HOST_ID" && "$(macos_json_get "$derived_path" writerGeneration 2>/dev/null || true)" == 1 && "$(macos_json_get "$derived_path" bootstrapPending 2>/dev/null || true)" == true && "$(macos_json_get "$derived_path" bootstrapIntentSha256 2>/dev/null || true)" == "$digest" ]] || macos_die "derived writer state changed outside the exact bootstrap intent"
      [[ -f "$derived_path.sha256" ]] || macos_write_checksum "$derived_path"
      [[ -f "$derived_path.sha256" ]] && macos_check_checksum "$derived_path"
    elif [[ "$derived_path" == "$identity_path" ]]; then
      macos_write_atomic "$derived_path" "$identity_json"
      macos_write_checksum "$derived_path"
    else
      macos_write_atomic "$derived_path" "$current_json"
      macos_write_checksum "$derived_path"
    fi
  done
}
...
bootstrap_validate_staging() {
  local staging_path="$1" commit checked_at relative_path validation_output
  [[ -n "$staging_path" ]] || macos_die "--staging-evidence is required for Activate"
  staging_path="$(macos_resolve_path "$staging_path")"
  [[ "$staging_path" == "$MACOS_LAYOUT_ROOT"/* && -f "$staging_path" && ! -L "$staging_path" ]] || macos_die "staging acceptance must be a canonical artifact inside the protected formal root"
  macos_secure_path "$staging_path"
  macos_check_checksum "$staging_path"
  plutil -convert json -o - -- "$staging_path" >/dev/null 2>&1 || macos_die "staging acceptance evidence is invalid"
  ...
  relative_path="/protected/${staging_path#$MACOS_LAYOUT_ROOT/}"
  ...
  commit="$(macos_json_get "$staging_path" commit 2>/dev/null || macos_json_get "$staging_path" gitCommit 2>/dev/null || true)"
  checked_at="$(macos_json_get "$staging_path" checkedAt 2>/dev/null || macos_json_get "$staging_path" checked_at 2>/dev/null || true)"
  [[ "$(macos_json_get "$staging_path" schemaVersion 2>/dev/null || true)" == 2 && "$(macos_json_get "$staging_path" kind 2>/dev/null || true)" == staging-acceptance && "$(macos_json_get "$staging_path" status 2>/dev/null || true)" == passed && "${commit:l}" == "$BOOTSTRAP_RELEASE_COMMIT" && "$(macos_json_get "$staging_path" hostOS 2>/dev/null || true)" == darwin && "$(macos_json_get "$staging_path" architecture 2>/dev/null || true)" == arm64 && "$(macos_json_get "$staging_path" platform 2>/dev/null || true)" == linux/arm64 ]] || macos_die "staging acceptance is not a passed schema-2 ARM64 acceptance for this release"
  [[ "$(macos_json_get "$staging_path" hostId 2>/dev/null || true)" == "$BOOTSTRAP_HOST_ID" ]] || macos_die "staging acceptance hostId does not match the pending writer"
  macos_assert_fresh_timestamp "$checked_at"
  [[ "$(macos_json_get "$staging_path" builtImageIdentitySha256 2>/dev/null || true)" == "$BOOTSTRAP_IMAGE_IDENTITY_SHA256" ]] || macos_die "staging acceptance image identity is stale"
  for gate in browser smtp capacity restart route security; do
    [[ "$(macos_json_get "$staging_path" "gates.$gate" 2>/dev/null || true)" == passed ]] || macos_die "staging acceptance gate is missing or failed: $gate"
  done
  typeset -g BOOTSTRAP_STAGING_PATH="$staging_path"
  typeset -g BOOTSTRAP_STAGING_SHA256="$(macos_sha256 "$staging_path")"
}
...
bootstrap_validate_restore_drill() {
  ...
  macos_secure_path "$restore_path"
...
bootstrap_validate_terminal_semantics() {
  ...
  if [[ "$current_generation" == 1 ]]; then
    [[ "$terminal_current_sha" == "$(macos_sha256 "$MACOS_CURRENT_STATE")" ]] || macos_die "activation terminal current-state digest binding is stale"
  fi
}
...
run_activate() {
  ...
  if [[ -f "$activation_terminal_path" ]]; then
    ...
    macos_check_checksum "$activation_intent_path"
    [[ -f "$phase_path" ]] || macos_die "activation terminal exists without its phase journal"
    if ! bootstrap_phase_read >/dev/null 2>&1; then
      bootstrap_repair_phase_sidecar
      bootstrap_phase_read
    fi
    [[ "$(macos_json_get "$phase_path" phase 2>/dev/null || true)" == terminal ]] || macos_die "activation terminal exists without a terminal phase journal"
    bootstrap_load_phase_artifacts
    bootstrap_load_terminal_artifacts
    bootstrap_validate_terminal_semantics
    if [[ ! -f "$activation_terminal_path.sha256" ]] || ! macos_check_checksum "$activation_terminal_path" >/dev/null 2>&1; then
